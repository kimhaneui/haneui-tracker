...
function setupGitCommitMessageControl(context: vscode.ExtensionContext) {
	...
	gitExtension.activate().then(() => {
		...
		if (repositories.length === 0) {
			...
			const onDidOpenRepository = git.onDidOpenRepository((repository: any) => {
...
function setupRepositoryCommitMessage(repository: any): void {
	...
	const analyzeChangesAndGetTemplate = (): string => {
	...
	// 커밋 메시지 변경 감지
	const onDidChange = inputBox.onDidChange((value: string) => {
