...
export class Dashboard {
	...
	constructor(context: vscode.ExtensionContext, calculator: StatsCalculator) {
		this.context = context;
		this.calculator = calculator;
	}

	show(data: any, period: string = 'daily'): void {
		if (this.panel) {
			this.panel.reveal();
		} else {
			...
			case 'daily':
				...
				if (!todayStats) {
					...
					stats = {
						...
						languages: Array.from<[string, number]>(todayStats.languages.entries())
							.map(([lang, count]) => ({
								language: lang,
								count,
	...
	private getWebviewContent(stats: any, period: string): string {
		...
		// 차트 데이터 준비
		const languageLabels = stats.languages.slice(0, 10).map((l: any) => l.language);
		const languageData = stats.languages.slice(0, 10).map((l: any) => l.count);
		const languageColors = this.generateColors(stats.languages.length);

		// 일별 코딩 시간 데이터
		const dayLabels = stats.days.map((d: any) => {
			const date = new Date(d.date);
			return `${date.getMonth() + 1}/${date.getDate()}`;
		});
		const dayCodingTime = stats.days.map((d: any) => d.codingTime / 3600); // 시간 단위로 변환
