...
export class StatsCalculator {
	...
	getWeeklyStats(data: any): any {
		...
		for (let i = 0; i < 7; i++) {
			...
			if (dayStats) {
				...
				dayStats.filesEdited.forEach((file: string) => allFilesEdited.add(file));
		...
		data.currentSession.filesEdited.forEach((file: string) => allFilesEdited.add(file));
	...
	getAllTimeStats(data: any): any {
		...
		return {
			...
			linesModified: totalLinesModified,
			languages,
			days: days.sort((a: any, b: any) => a.date.localeCompare(b.date)),
		};
	}
